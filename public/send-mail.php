...
$smtpHost = config_value('FYK_SMTP_HOST', 'mail.fykconstructions.com');
$smtpPort = (int) config_value('FYK_SMTP_PORT', 587);
$smtpUser = config_value('FYK_SMTP_USER', 'noreply@fykconstructions.com');
$smtpPass = config_value('FYK_SMTP_PASS');
$fromEmail = config_value('FYK_FROM_EMAIL', 'noreply@fykconstructions.com');
$fromName = config_value('FYK_FROM_NAME', 'FYK Construction Website');
$toEmail = config_value('FYK_CONTACT_TO', 'oosthuizenp@gmail.com');
$bccEmail = config_value('FYK_CONTACT_BCC');
$debugMode = isset($_GET['debug']) || getenv('FYK_MAIL_DEBUG') === '1';
...
$organisation = clean_text($payload['organisation'] ?? '');
$location = clean_text($payload['location'] ?? '');
$service = clean_text($payload['service'] ?? '');
$preference = clean_text($payload['preference'] ?? '');
...
if ($type === 'newsletter') {
    ...
    $mailSubject = 'Newsletter Sign-Up | FYK Construction Website';
    ...
    if ($service !== '') {
        $mailBody .= "Service required: {$service}\n";
    }

    if ($preference !== '') {
        $mailBody .= "Preferred contact method: {$preference}\n";
    }
